...
res$model$Model$base_class <- "Base"
...
for (nm in setdiff(names(res3), "Base")) {
  message(nm)
  a <- res3[[nm]]
  cat(get_class_string(a, res2), file = "R/bokeh_models.R", append = TRUE)
  cat(get_test_string(a), file = "tests/testthat/test-models.R", append = TRUE)
}
